...
class TestKunjunganPcareFixed
{
    use PcareTrait;
    
    public function testKunjungan($noRawat)
    {
        echo "=== TEST KUNJUNGAN PCARE - FORMAT DIPERBAIKI ===\n\n";
        ...
        try {
            ...
            // 7. Persiapkan data kunjungan dengan format yang diperbaiki
            // PERBAIKAN: Mengganti semua null dengan string kosong atau nilai default
            $kunjunganData = [
                'noKunjungan' => '', // Ganti null dengan string kosong
                'noKartu' => (string)$dataPasien->no_peserta,
                'tglDaftar' => date('d-m-Y', strtotime($dataPasien->tgl_registrasi)),
                'kdPoli' => (string)$kdPoliPcare,
                'keluhan' => (string)($pemeriksaanData->keluhan ?? 'Kontrol rutin'),
                'kunjSakit' => true,
                ...
                'kdDokter' => (string)($dataPasien->kd_dokter_pcare ?? ''), // Ganti null dengan string kosong
                'kdDiag1' => (string)($diagnosaData->kd_penyakit ?? 'Z00.0'),
                'kdDiag2' => '', // Ganti null dengan string kosong
                'kdDiag3' => '', // Ganti null dengan string kosong
                'kdPoliRujukInternal' => '', // Ganti null dengan string kosong
                'rujukLanjut' => '', // Ganti null dengan string kosong
                'kdTacc' => -1,
                'alasanTacc' => '', // Ganti null dengan string kosong
                'anamnesa' => (string)($pemeriksaanData->anamnesis ?? 'Pemeriksaan rutin'),
                'alergiMakan' => '00',
                'alergiUdara' => '00',
                'alergiObat' => '00',
                'kdPrognosa' => '01', // Baik
                'terapiObat' => (string)$terapiObatString,
                'terapiNonObat' => (string)($pemeriksaanData->instruksi ?? 'Kontrol rutin'),
                'bmhp' => 'Tidak ada',
                'suhu' => (string)($pemeriksaanData->suhu_tubuh ?? '36.5')
            ];
            
            echo "📤 Data Kunjungan yang Akan Dikirim (Format Diperbaiki):\n";
            ...
            Log::info('Testing PCare Kunjungan - Fixed Format', [
                'no_rawat' => $noRawat,
            ...
            if (isset($responseData['metaData'])) {
                ...
                if ($httpCode == 200 || $httpCode == 201) {
                    ...
                    } elseif (isset($responseData['response']['field']) && $responseData['response']['field'] == 'noKunjungan') {
                        echo "   - No Kunjungan: {$responseData['response']['message']}\n";
                    }
                } elseif ($httpCode == 412) {
                    ...
                    echo "   - Mengganti semua nilai null dengan string kosong\n";
                    echo "   - Memastikan semua field bertipe data yang benar\n";
                    echo "   - Menggunakan casting eksplisit untuk string, int, float\n";
                } else {
...
// Jalankan test
$test = new TestKunjunganPcareFixed();
