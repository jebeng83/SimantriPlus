  ...
  return (
    <div className="mt-6 border rounded overflow-hidden">
      <div className="px-4 py-3 bg-gray-50 border-b flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-700">Ringkasan Pengaturan Per Barang</h4>
        {!hasData && <span className="text-xs text-gray-500">Belum ada pengaturan per barang</span>}
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-3 py-2 text-left">Kode Barang</th>
              {columns.map((c) => (
          ...
          <tbody>
            {hasData ? (
              items.map((row) => (
                <tr key={row.kode_brng} className="border-t">
                  <td className="px-3 py-2">{String(row.kode_brng || '').toUpperCase()}</td>
                  {columns.map((c) => (
